...
test.describe('Misc API routes', () => {
  test('health returns status', async ({ authedRequest }) => {
    const res = await authedRequest.get('/api/v1/health');
  ...
  test('tags returns array', async ({ authedRequest }) => {
    const res = await authedRequest.get('/api/v1/tags');
    expect(res.status()).toBe(200);
    const body = await res.json();
  ...
  test('handles/check available', async ({ authedRequest }) => {
    const res = await authedRequest.get('/api/v1/handles/check?handle=zzz-nonexistent-handle-999');
    expect(res.status()).toBe(200);
    const body = await res.json();
  ...
  test('handles/check taken', async ({ authedRequest }) => {
    const res = await authedRequest.get('/api/v1/handles/check?handle=e2e-test');
    expect(res.status()).toBe(200);
    const body = await res.json();
  ...
  test('handles/check missing param', async ({ authedRequest }) => {
    const res = await authedRequest.get('/api/v1/handles/check');
    expect(res.status()).toBe(400);
  });

  test('feedback creates entry', async ({ authedRequest }) => {
    const res = await authedRequest.post('/api/v1/feedback', {
