...
test.describe('Podcast CRUD API routes', () => {
  ...
  test('list own podcasts', async ({ authedRequest }) => {
    const res = await authedRequest.get('/api/podcasts');
  ...
  test('patch title', async ({ authedRequest, seedData }) => {
    const res = await authedRequest.patch(`/api/podcasts/${seedData.testPodcast.id}`, {
    ...
    await authedRequest.patch(`/api/podcasts/${seedData.testPodcast.id}`, {
      data: { title: 'E2E Test Podcast' },
    });
  });

  test('patch non-owned returns 403', async ({ authedRequest, seedData }) => {
    const res = await authedRequest.patch(`/api/podcasts/${seedData.otherPodcast.id}`, {
  ...
  test('versions returns list', async ({ authedRequest, seedData }) => {
    const res = await authedRequest.get(`/api/podcasts/${seedData.testPodcast.id}/versions`);
    expect(res.status()).toBe(200);
    const body = await res.json();
