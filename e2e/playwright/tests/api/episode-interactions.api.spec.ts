...
test.describe('Episode interactions API routes', () => {
  test.describe.configure({ mode: 'serial' });

  test('create interaction', async ({ authedRequest, seedData }) => {
    const res = await authedRequest.post(`/api/v1/episodes/${seedData.testEpisode.id}/interact`, {
      data: {
  ...
  test('incorporate returns 202 or 403', async ({ authedRequest, seedData }) => {
    const res = await authedRequest.post(
      `/api/v1/episodes/${seedData.testEpisode.id}/interact/${seedData.interaction.id}/incorporate`
    );
