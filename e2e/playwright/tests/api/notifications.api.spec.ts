...
test.describe('Notifications API routes', () => {
  test('GET notifications', async ({ authedRequest }) => {
    const res = await authedRequest.get('/api/notifications');
  ...
  test('PATCH mark notification read', async ({ authedRequest, seedData }) => {
    ...
    const res = await authedRequest.patch(`/api/notifications/${notifId}`, {
      data: { read: true },
    });
    expect(res.status()).toBe(200);
  });
  ...
  test('POST mark all read', async ({ authedRequest }) => {
    const res = await authedRequest.post('/api/notifications/mark-all-read');
    expect(res.status()).toBe(200);
  });
});
