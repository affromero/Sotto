...
- **Flexible language pairs** — English→German, English→Spanish, and Spanish→English ship as hand-authored reference curricula that set the quality bar; other selected native→target pairs are composed by your configured AI/agent on demand and cached per pair.
...
The newest wave is LLM-native, and the closest peer is genuinely good: [**OpenLingo**](https://github.com/pretzelai/openlingo) is also open-source ([MIT](https://github.com/pretzelai/openlingo/blob/main/LICENSE)), self-hostable, and BYO-LLM, with [SM-2](https://super-memory.com/english/ol/sm2.htm) spaced repetition, [Whisper](https://openai.com/research/whisper) speaking exercises, and a similar stack ([Next.js](https://nextjs.org/) 16 / [React](https://react.dev/) 19 / [PostgreSQL](https://www.postgresql.org/) 16). Credit where due. **Where Sotto goes further:** a structured, [mastery-gated](https://en.wikipedia.org/wiki/Mastery_learning) CEFR course across five _graded_ skills (including writing with inline corrections and rubric-based pronunciation scoring), a **[keyless local-agent](#bring-your-own-claude--codex) path** (run it through your own Claude Code / Codex with no API key), a **[100%-offline path](#run-it-100-offline)** (local LLM + STT + TTS — no cloud key for _any_ layer), an interactive memory graph, adaptive listening seeded by your due vocabulary, a first-party terminal client, and Pencil-first iPad workbooks.
...
| Runs 100% offline ([local LLM](#run-it-100-offline) + STT + TTS, no cloud key)                                                          |                 ✅                 |                          —                          |                  —                   |               —                |                —                |               —               |                  —                   |
| Multi-user households on your server ([invite your family](#your-devices-and-household))                                                |                 ✅                 |                         〰️                          |                  —                   |               —                |                —                |               —               |                  —                   |
| First-party terminal client / TUI for the full learning loop                                                                            |                 ✅                 |                          —                          |                  —                   |               —                |                —                |               —               |                  —                   |
| Pencil-first iPad workbook PDF for each class                                                                                           |                 ✅                 |                          —                          |                  —                   |               —                |                —                |               —               |                  〰️                  |
| 5 _graded_ skills (grammar / reading / listening / speaking / writing)                                                                  |                 ✅                 |                         〰️                          |                  〰️                  |               〰️               |               〰️                |              〰️               |                  〰️                  |
| Mastery-gated progression                                                                                                               |                 ✅                 |                          —                          |                  〰️                  |               —                |               〰️                |              〰️               |                  〰️                  |
| [Spaced repetition](https://en.wikipedia.org/wiki/Spaced_repetition) / review scheduling                                                |                 ✅                 |                         ✅                          |                  ✅                  |               —                |                —                |               —               |                  〰️                  |
| Interactive memory graph                                                                                                                |                 ✅                 |                          —                          |                  —                   |               —                |                —                |               —               |                  —                   |
| Adaptive listening seeded by due vocab                                                                                                  |                 ✅                 |                          —                          |                  —                   |               —                |                —                |               —               |                  〰️                  |
| Classes from your own sources, with verified `[N]` citations                                                                            |                 ✅                 |                         〰️                          |                  —                   |               —                |               〰️                |              〰️               |                  —                   |
| Institutional-style practice exams (Goethe / DELE / Cambridge format)                                                                   |                 ✅                 |                          —                          |                  —                   |               —                |               〰️                |               —               |                  —                   |
| AI conversation / roleplay                                                                                                              |                 ✅                 |                         ✅                          |                  〰️                  |               ✅               |               ✅                |              ✅               |                  〰️                  |
| Real-time spoken translation practice                                                                                                   |                 ✅                 |                          —                          |                  —                   |               —                |                —                |               —               |                  —                   |
| Rubric pronunciation scoring                                                                                                            |                 ✅                 |                          —                          |                  〰️                  |               ✅               |               〰️                |              〰️               |                  —                   |
| Price                                                                                                                                   | Self-host (pay providers directly) |                     Free (MIT)                      |            Freemium + sub            |              Sub               |               Sub               |              Sub              |                 Sub                  |

<sub>✅ publicly documented full support · 〰️ partial, adjacent, or limited support · ❌ public product model indicates no · — not found in the public docs checked; this is not proof the feature is impossible, especially for closed apps with regional experiments. Values reflect public repo/site/App Store/Google Play/help pages checked in June 2026. OpenLingo: [`pretzelai/openlingo`](https://github.com/pretzelai/openlingo) documents MIT OSS, Docker/PostgreSQL setup, provider API keys, SM-2 SRS, AI tutor conversations, translated reading material, OpenAI TTS, and Whisper STT; no public terminal client, Pencil workbook, local no-cloud STT/TTS path, mastery gates, memory graph, or rubric pronunciation score found. Duolingo publicly documents CEFR/ACTFL alignment via Duolingo for Schools, spaced repetition research/use, and Max/Explain My Answer/Roleplay, but not self-hosting, BYOK, workbook export, or source-grounded class generation. Speak, Praktika, and TalkPal publicly emphasize AI conversation and feedback; the table only gives full credit where the specific capability is named clearly. _Workbooks:_ Pimsleur offers [replacement reading booklet PDFs](https://www.pimsleur.com/pimsleur-lost-and-found/), so it earns 〰️ there, but not a generated Pencil-first class workbook with web-class deep links. Sotto values reflect the self-hosted OSS build; corrections welcome via PR.</sub>
...
| -------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`packages/groundcheck`](packages/groundcheck/README.md)       | Domain-aware reference verification. A Nature paper, a Reuters article, and a government report should not be judged by the same citation formula, so `groundcheck` classifies the source domain and combines verification evidence with domain-specific weights/Bayesian scoring. |
| [`packages/learning-model`](packages/learning-model/README.md) | Learner mastery and practice recommendation. It models a learner's mastery from practice evidence and ranks weak, uncertain, due, or repeatedly missed targets so catch-up practice stays focused on learning value rather than easy repetition.                                   |
