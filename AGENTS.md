...
- Do not require a hosted secret manager for local OSS workflows.
...
Critical local variables are `DATABASE_URL`, `REDIS_URL`, `BYOK_ENCRYPTION_KEY`, and `SIDEDOOR_EXECUTION_DIR`. Configure providers, credentials, and local/R2/S3 storage through `/welcome` or Admin.
