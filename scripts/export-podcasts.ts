#!/usr/bin/env npx tsx
/**
 * Export local podcasts as a SQL migration file that can be run on production.
 *
 * Usage:
 *   npx tsx scripts/export-podcasts.ts > migration.sql
...
async function main() {
  ...
  const [scripts, segments, references, versions, discoveries, interactions] = await Promise.all([
    prisma.script.findMany({ where: { podcastId: { in: podcastIds } } }),
    prisma.segment.findMany({ where: { podcastId: { in: podcastIds } }, orderBy: { order: 'asc' } }),
    prisma.reference.findMany({ where: { podcastId: { in: podcastIds } } }),
    prisma.podcastVersion.findMany({ where: { podcastId: { in: podcastIds } }, include: { segments: true } }),
    prisma.discovery.findMany({ where: { podcastId: { in: podcastIds } }, include: { messages: true } }),
    prisma.interaction.findMany({ where: { podcastId: { in: podcastIds } } }),
  ]);

  // Also fetch tags for these podcasts
  const podcastTags = await prisma.podcastTag.findMany({ where: { podcastId: { in: podcastIds } } });
  const tagIds = [...new Set(podcastTags.map(pt => pt.tagId))];
  ...
  // Build SQL
  const lines: string[] = [];
  lines.push('-- Auto-generated podcast migration');
  lines.push(`-- Source: local DB, ${podcasts.length} podcasts`);
  ...
  for (const p of podcasts) {
    ...
    lines.push(insertRow('Podcast', row, true));
  }
  lines.push('');
  ...
  // Podcast Tags
  if (podcastTags.length > 0) {
    lines.push('-- Podcast Tags');
    for (const pt of podcastTags) {
      const row: Record<string, unknown> = { ...pt };
      lines.push(insertRow('PodcastTag', row));
    }
    lines.push('');
  }
