...
| `stripe.ts`              | Stripe client, TIER_LIMITS (FREE/STARTER/PRO/STUDIO/ADMIN), `INTERACTION_CREDIT_COST` (0.25), `sharedVoiceSurcharge`, `canGenerate()` (accepts sharedVoiceCount), `canInteract()` (credit-based), checkout | Stripe                                        |
| `credits.ts`             | Credit operations: `consumeCredit()`, `refundCredits()`, `addPurchasedCredits()`, `grantMonthlyCredits()`                                                                                                  | Uses `prisma.ts`                              |
...
| `subscription.ts`        | `getUserTier()`, `getUserUsage()`, `getUserVoiceCredits()`, `resetMonthlyUsage()` (credit-based)                                                                                                           | Uses `prisma.ts` + `stripe.ts` + `credits.ts` |
