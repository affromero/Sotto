...
import { getEpisodeGradient } from '../../lib/gradients';
import type { EpisodeDetail, SegmentData } from '@sotto/shared';
...
export default function EpisodeScreen() {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      if (episode?.isSaved) {
        await api.delete(`/episodes/${id}/save`);
      } else {
        await api.post(`/episodes/${id}/save`);
      }
    },
    onMutate: async () => {
      await queryClient.cancelQueries({ queryKey: ['episode', id] });
      const previous = queryClient.getQueryData<EpisodeDetail>(['episode', id]);
      if (previous) {
        queryClient.setQueryData<EpisodeDetail>(['episode', id], {
          ...previous,
    ...
    onError: (_err, _vars, context) => {
      if (context?.previous) {
        queryClient.setQueryData(['episode', id], context.previous);
      }
    },
    onSettled: () => {
  ...
  const interactMutation = useMutation({
    mutationFn: async (payload: { question: string; timestamp: number }) => {
      const res = await api.post(`/episodes/${id}/interact`, payload);
  ...
  // Load track when episode data arrives and player is ready
  useEffect(() => {
    if (!playerReady || !episode?.audioUrl) return;
    loadTrack(episode.id, episode.audioUrl, episode.title, episode.user?.name ?? 'Sotto').catch(
      () => {}
    );
    setCurrentEpisode({
      id: episode.id,
      title: episode.title,
      creator: episode.user?.name ?? 'Sotto',
  ...
  const handleSeek = useCallback(
    async (ratio: number) => {
      if (!playerReady) return;
      const totalDuration = episode?.duration ?? trackDuration;
      if (totalDuration > 0) {
        try {
    ...
    [episode?.duration, trackDuration, playerReady]
  );
  ...
  const currentSegmentIndex = episode?.segments
    ? findCurrentSegmentIndex(episode.segments ?? [], position)
    : -1;
    ...
    return (
      <View style={styles.centered} testID="generation-error">
        ...
        <Pressable
          ...
          onPress={() => queryClient.invalidateQueries({ queryKey: ['episode', id] })}
  ...
  const isOwner = currentUser?.id === episode.user?.id;

  // Show failure details for owners when generation failed
  if (episode.status === 'FAILED' && isOwner) {
    return (
      <View style={styles.centered} testID="generation-failed">
        ...
        <Text style={styles.errorSubtext}>
          {episode.failureReason ?? 'An unexpected error occurred during generation.'}
        </Text>
        <Pressable
          style={styles.retryButton}
          onPress={async () => {
            try {
              await api.post(`/episodes/${id}/script/regenerate`);
              queryClient.invalidateQueries({ queryKey: ['episode', id] });
        ...
        <Pressable
          ...
          onPress={() => {
            Alert.alert(
              'Delete Episode',
              'This will permanently delete this episode. Are you sure?',
              [
                { text: 'Cancel', style: 'cancel' },
                {
                  ...
                  onPress: async () => {
                    try {
                      ...
                      Alert.alert('Error', 'Could not delete episode.');
                    }
                  },
                },
              ]
  ...
  const gradient = getEpisodeGradient(episode.id);
  ...
  const listHeader = (
    <>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.creator}>{episode.user?.name ?? 'Unknown Creator'}</Text>
        <View style={styles.episodeBadgeRow}>
          <View style={styles.episodeContentBadge}>
            <Text style={styles.episodeContentBadgeText}>
              {getContentBadgeLabel(episode)}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.actionRow}>
          ...
          <View style={styles.actionIcons}>
            <Pressable
              ...
              accessibilityLabel={episode.isSaved ? 'Unsave episode' : 'Save episode'}
              accessibilityRole="button"
              testID="player-save-button"
            >
              <Animated.View style={saveAnimatedStyle}>
                <Ionicons
                  name={episode.isSaved ? 'bookmark' : 'bookmark-outline'}
                  size={22}
                  color={episode.isSaved ? colors.primary : colors.textSecondary}
                />
              </Animated.View>
              {isOwner && <Text style={styles.actionCount}>{episode.saveCount}</Text>}
            </Pressable>
            ...
            {isOwner && (
              <Pressable
                onPress={() => router.push(`/episode/${episode.id}/edit`)}
                style={styles.actionIcon}
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList<SegmentData>
        ...
        ListFooterComponent={
          <>{episode.references?.length > 0 && <ReferencesTab references={episode.references} />}</>
        }
      ...
      >
        <Pressable
          ...
          accessibilityLabel="Ask a question about this episode"
          accessibilityRole="button"
