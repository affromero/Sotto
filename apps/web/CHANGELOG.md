...
- `GET /video?summary=true` endpoint returns status of all video generations for a podcast

### Changed
- VideoGeneration uses compound unique `@@unique([podcastId, voiceTrackId])` instead of `podcastId @unique`
