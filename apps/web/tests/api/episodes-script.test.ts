...
import { GET, PATCH } from '@/app/api/v1/episodes/[episodeId]/script/route';

function createGetRequest(): NextRequest {
  return new NextRequest(new URL('http://localhost:3000/api/v1/episodes/pod-1/script'));
}
...
describe('GET /api/v1/episodes/[episodeId]/script', () => {
  ...
  it('returns 403 when user does not own the episode', async () => {
    mockAuthenticateRequest.mockResolvedValue({ userId: 'user-1' });
    mockEpisodeFindUnique.mockResolvedValue({ userId: 'other-user' });
  ...
  it('returns script turns, references, and version on success', async () => {
    mockAuthenticateRequest.mockResolvedValue({ userId: 'user-1' });
    mockEpisodeFindUnique.mockResolvedValue({ userId: 'user-1', lowReferences: false });
    const turns = [
  ...
  it('includes lowReferences and requiredRefCount when episode has low references', async () => {
    mockAuthenticateRequest.mockResolvedValue({ userId: 'user-1' });
