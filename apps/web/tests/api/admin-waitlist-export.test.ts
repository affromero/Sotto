...
describe('GET /api/admin/waitlist/export', () => {
  ...
  it('returns CSV with waitlist entries', async () => {
    ...
    const entries = [
      { email: 'alice@test.com', twitterHandle: 'alice', source: 'landing', status: 'APPROVED', createdAt: new Date('2026-01-15T10:00:00Z'), approvedAt: new Date('2026-01-16T00:00:00Z'), signedUpAt: null },
      { email: 'bob@test.com', twitterHandle: null, source: null, status: 'PENDING', createdAt: new Date('2026-01-16T12:00:00Z'), approvedAt: null, signedUpAt: null },
    ];
    ...
    expect(text).toContain('Email,Twitter,Source,Status,Signed Up,Approved At,Converted At');
    expect(text).toContain('alice@test.com,alice,landing,APPROVED,');
  ...
  it('returns CSV with only header when no entries', async () => {
    ...
    expect(response.status).toBe(200);
    expect(text).toBe('Email,Twitter,Source,Status,Signed Up,Approved At,Converted At');
  });
