...
describe('PATCH /api/users/me', () => {
  ...
  it('updates briefing customization fields', async () => {
    mockAuthenticateRequest.mockResolvedValue({ userId: 'user-1' });
    mockPrisma.user.update.mockResolvedValue({
      ...mockUser,
      briefingDepth: 'deep_dive',
      briefingTone: 'professional',
      briefingAudienceLevel: 'expert',
      briefingDuration: 15,
      briefingPrompt: 'Focus on AI research',
    });
  ...
  it('clears briefing fields when set to null', async () => {
    mockAuthenticateRequest.mockResolvedValue({ userId: 'user-1' });
    mockPrisma.user.update.mockResolvedValue(mockUser);

    const request = createPatchRequest({
      briefingAiModel: null,
      briefingTtsProvider: null,
      briefingTtsModel: null,
      briefingDepth: null,
      briefingTone: null,
      briefingAudienceLevel: null,
      briefingDuration: null,
      briefingPrompt: null,
      briefingHostVoiceId: null,
      briefingExpertVoiceId: null,
    });
    const response = await PATCH(request);
  ...
  it('validates briefingAiModel against registry', async () => {
    mockAuthenticateRequest.mockResolvedValue({ userId: 'user-1' });

    const request = createPatchRequest({ briefingAiModel: 'nonexistent-model' });
    const response = await PATCH(request);
    const body = await response.json();
  ...
  it('accepts valid briefingAiModel', async () => {
    mockAuthenticateRequest.mockResolvedValue({ userId: 'user-1' });
    mockPrisma.user.update.mockResolvedValue({
      ...mockUser,
      briefingAiModel: 'claude-sonnet-4-6',
    });

    const request = createPatchRequest({ briefingAiModel: 'claude-sonnet-4-6' });
    const response = await PATCH(request);

    expect(response.status).toBe(200);
  });

  it('rejects invalid briefingDepth enum value', async () => {
    mockAuthenticateRequest.mockResolvedValue({ userId: 'user-1' });

    const request = createPatchRequest({ briefingDepth: 'invalid_depth' });
    const response = await PATCH(request);

    expect(response.status).toBe(400);
  });

  it('rejects invalid briefingTone enum value', async () => {
    mockAuthenticateRequest.mockResolvedValue({ userId: 'user-1' });

    const request = createPatchRequest({ briefingTone: 'angry' });
    const response = await PATCH(request);

    expect(response.status).toBe(400);
  });

  it('rejects briefingDuration outside valid range', async () => {
    mockAuthenticateRequest.mockResolvedValue({ userId: 'user-1' });

    const request = createPatchRequest({ briefingDuration: 50 });
    const response = await PATCH(request);

    expect(response.status).toBe(400);
  });

  it('rejects briefingPrompt exceeding 2000 chars', async () => {
    mockAuthenticateRequest.mockResolvedValue({ userId: 'user-1' });

    const request = createPatchRequest({ briefingPrompt: 'a'.repeat(2001) });
    const response = await PATCH(request);

    expect(response.status).toBe(400);
  });

  it('updates briefingUseByokKeys toggle', async () => {
    mockAuthenticateRequest.mockResolvedValue({ userId: 'user-1' });
    mockPrisma.user.update.mockResolvedValue({
      ...mockUser,
      briefingUseByokKeys: false,
    });

    const request = createPatchRequest({ briefingUseByokKeys: false });
    const response = await PATCH(request);
