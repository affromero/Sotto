...
| Free Tier Config  | FreeTierConfig                                                                                                                                                  | Singleton row: admin-configurable free tier settings (AI provider/model, TTS provider, daily generation limit)                         |
| Auto Model Config | AutoModelConfig                                                                                                                                                 | Singleton row: per-plan "Auto" model resolution (Free + Pro AI/TTS/STT provider+model defaults, `freeIncludedModels`/`proIncludedModels` Json? for AI picker, `freeIncludedTtsModels`/`proIncludedTtsModels` Json? for TTS picker, `freeIncludedSttModels`/`proIncludedSttModels` Json? for STT picker — all use provider:model composite IDs for TTS/STT; `proImageProvider`/`proImageModel`/`proIncludedImageModels` for video pipeline image generation) |
