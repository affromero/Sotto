...
| `visual-generation`         | `visual-generation`         | 5      | SegmentVisual → AI illustration (fal FLUX), stock footage (Pexels), or map image (Mapbox from pre-enriched places) → upload asset to R2 | Updates SegmentVisual.assetUrl + status, queues video-composition when all segments ready |
