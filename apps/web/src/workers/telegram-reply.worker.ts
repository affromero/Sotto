...
export async function processTelegramReply(job: Job<ReplyTelegramPayload>): Promise<void> {
  ...
  if (podcast.status === 'FAILED') {
    try {
      await sendMessage(
        chatId,
        `Sorry, we couldn't generate your podcast. Try again or visit ${appUrl} to create one manually.`
      );
  ...
  await sendMessage(chatId, `Your podcast is ready! "${podcast.title}"${durationStr}`, {
    reply_markup: {
