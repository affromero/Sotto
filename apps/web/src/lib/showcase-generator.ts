...
async function generateShowcaseMetadata(topic: string, aiModel: string): Promise<{ segments: GeneratedSegment[]; hints: ExternalHints }> {
  ...
  const d = diagram;
  const svgContent = `<svg viewBox="0 0 600 400" xmlns="http://www.w3.org/2000/svg"><rect width="600" height="400" fill="#2A3550" rx="20"/><text x="300" y="50" text-anchor="middle" font-family="IBM Plex Sans,sans-serif" font-size="22" font-weight="bold" fill="white">${d.title ?? topic}</text><circle cx="300" cy="200" r="80" fill="none" stroke="#3F4FB0" stroke-width="4"/><circle cx="300" cy="200" r="50" fill="rgba(63,79,176,0.2)" stroke="#3F4FB0" stroke-width="2"/><text x="300" y="205" text-anchor="middle" fill="#3F4FB0" font-size="14" font-family="IBM Plex Sans,sans-serif" font-weight="600">${d.centerLabel ?? ''}</text><text x="300" y="90" text-anchor="middle" fill="#9CA3AF" font-size="13" font-family="IBM Plex Sans,sans-serif">${d.topLabel ?? ''}</text><text x="120" y="350" text-anchor="middle" fill="white" font-size="14" font-family="IBM Plex Sans,sans-serif">${d.bottomLeftLabel ?? ''}</text><text x="480" y="350" text-anchor="middle" fill="white" font-size="14" font-family="IBM Plex Sans,sans-serif">${d.bottomRightLabel ?? ''}</text><line x1="120" y1="335" x2="200" y2="280" stroke="#6B7280" stroke-width="2" stroke-dasharray="6"/><line x1="480" y1="335" x2="400" y2="280" stroke="#6B7280" stroke-width="2" stroke-dasharray="6"/><text x="300" y="380" text-anchor="middle" fill="#6B7280" font-size="12" font-family="IBM Plex Sans,sans-serif">${d.subtitle ?? ''}</text></svg>`;
