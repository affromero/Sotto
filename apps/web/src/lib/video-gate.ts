...
// ── Avatar generation gate (separate daily counter, same limits as video) ──

async function getAvatarDailyCount(userId: string): Promise<{ count: number; ttl: number }> {
  const redis = getRedisClient();
  const key = `free:avatar:daily:${userId}`;
  const [countStr, ttl] = await Promise.all([redis.get(key), redis.ttl(key)]);
  return { count: parseInt(countStr ?? '0', 10), ttl };
}
...
export async function checkAvatarGenerationGate(userId: string): Promise<VideoGateResult> {
  ...
  if (dailyUsed >= dailyLimit) {
    return {
      ...
      dailyUsed,
      dailyRemaining: 0,
      resetInSeconds: ttl > 0 ? ttl : 86400,
    };
  }
...
export async function tryIncrementAvatarGeneration(
  userId: string,
  limit: number,
): Promise<boolean> {
  const redis = getRedisClient();
