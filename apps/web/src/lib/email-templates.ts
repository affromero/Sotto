...
export function buildWaitlistApprovalEmail(email: string): { subject: string; html: string } {
  ...
  const twitterTip =
    twitterBotHandle && twitterBotUrl
      ? `<div style="background:#F5F4F0; border:1px solid #f3f4f6; border-radius:8px; padding:16px; margin-top:24px;">
          <p style="font-size:13px; line-height:1.6; color:#6B7280; margin:0 0 8px;">
            <strong style="color:#1E2128;">Quick tip:</strong> Tag
            <a href="${twitterBotUrl}" style="color:#3F4FB0; text-decoration:none; font-weight:600;">${twitterBotHandle}</a>
          ...
          <p style="font-size:12px; line-height:1.5; color:#9ca3af; margin:0;">
            Just <a href="${appUrl}/settings" style="color:#3F4FB0; text-decoration:none;">link your X account</a> in settings after signing up.
          </p>
        </div>`
      : '';
  return {
