...
export async function MyPodcastsSection({ userId, userRole }: MyPodcastsSectionProps) {
  const [podcasts, userInfo, freeTier] = await Promise.all([
  ...
  const userHandle = userInfo?.handle;
      ...
      <section className={styles.podcastsSection}>
        ...
        {podcasts.length === 0 ? (
          <div className={styles.emptyState}>
            ...
            <Link href="/create" className={styles.emptyCta}>Get Started</Link>
          </div>
        ) : (
            ...
            {podcasts.map((podcast) => {
              ...
              return (
                <div key={podcast.id} className={`${styles.cardWrapper} dashboardCardWrapper`} role="listitem">
                  ...
                  >
                    <div className={`${styles.miniGradientCover} ${podcast.status === 'FAILED' ? styles.miniGradientFailed : ''} ${podcast.status === 'DRAFT' ? styles.miniGradientDraft : ''}`}>
                      <div className={styles.miniGradientBadge}>
                        <Badge variant={statusVariants[podcast.status]}>{statusLabels[podcast.status]}</Badge>
                      </div>
                      ...
                      <div className={styles.miniGradientMeta}>
                        {podcast.status === 'DRAFT' ? (
                          ...
                          <>
                            ...
                            {podcast.playCount > 0 && <span>{podcast.playCount.toLocaleString()} plays</span>}
                          </>
                        )}
                      </div>
                      {podcast.status === 'DRAFT' && <span className={styles.draftHint}>Tap to continue</span>}
                      {podcast.status === 'FAILED' && (
                        <>
                          {isAdmin && podcast.failureReason && <p className={styles.failureReason}>{podcast.failureReason}</p>}
