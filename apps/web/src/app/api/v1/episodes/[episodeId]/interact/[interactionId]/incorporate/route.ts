...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  const interaction = await prisma.interaction.findUnique({
    ...
    include: {
      episode: { select: { id: true, userId: true, status: true, source: true, language: true, aiModel: true } },
    },
  });
  ...
  if (interaction.episode.status !== 'READY') {
    return errorResponse(`Episode is currently "${interaction.episode.status}", must be READY`, 409);
  }
  ...
  const providerAiKey =
    interaction.episode.aiModel && provider !== 'claude-code'
      ? await getAiKey(userId, provider as AiProviderId)
      : aiKey;
  if (interaction.episode.aiModel && provider !== 'claude-code' && !providerAiKey) {
