...
/**
 * POST — Trigger video generation for a READY episode.
 */
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  // Feature gate: check provider availability.
      ...
      if (visuals.length === 0) {
        ...
        if (allReady) {
          ...
          await addJob(videoCompositionQueue, JobType.COMPOSE_VIDEO, {
            episodeId, videoGenerationId: existing.id,
          });
          return NextResponse.json({
  ...
  if (pipeline && episode.zeroCostVideo) {
  ...
  const videoGeneration = await prisma.videoGeneration.create({
    data: {
      ...
      zeroCostVideo: episode.zeroCostVideo,
  ...
  if (pipeline) {
    ...
    if (externalVisuals.length > 0) {
      ...
      // All visuals programmatic — queue composition
      await addJob(videoCompositionQueue, JobType.COMPOSE_VIDEO, {
        episodeId,
        videoGenerationId: videoGeneration.id,
      });
    ...
    logger.info('Video generation started from pipeline', { episodeId, segmentCount: String(pipeline.segments.length) });
  } else {
...
export async function GET(_request: NextRequest, { params }: RouteParams) {
  ...
  // Public episodes: anyone can read video data. Private: owner/admin only.
  const isPublicEpisode = episode.visibility === 'PUBLIC';
  const isOwner = authResult?.userId === episode.userId;
  const adminId = await requireAdmin();
  if (!isPublicEpisode && !isOwner && !adminId) {
    return errorResponse('Forbidden', 403);
  }
  ...
  // Summary mode: return status of the current generation for this episode
  if (summary) {
...
export async function DELETE(_request: NextRequest, { params }: RouteParams) {
  ...
  logger.info('Video generation deleted', { episodeId });
...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  ...
  if (allReady) {
    ...
    logger.info('Video segments updated (all programmatic)', { episodeId, count: String(segmentIds.length) });
    return NextResponse.json({ videoGenerationId: videoGeneration.id, status: 'READY' });
  }

  logger.info('Video segments updated, regenerating', { episodeId, count: String(segmentIds.length) });
