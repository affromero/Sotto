...
| `landing/chapters/` | HeroChapter, JourneyChapter (+ ScriptEditorMock, AudioClipPlayer), ShowcaseChapter, IdentityChapter, TrustChapter, NetworkChapter, BotChapter, FaqChapter, ConvertChapter | Landing page chapters: hero, journey (data-driven chat/script/player from LandingShowcase config), showcase (data-driven video segments), identity (verified voice cloning + avatar images), bot (real Twitter/Telegram links), trust, network, faq (accordion with personality — BYOK/Fal, creator protection, anti-slop philosophy, learning retention), convert. AudioClipPlayer embeds synced video inline. All chapters fall back to hardcoded content when no LandingShowcase config exists |
