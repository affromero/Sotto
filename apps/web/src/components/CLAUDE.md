...
| `player/`        | AudioPlayer, MiniPlayer, Waveform, PlaybackControls, TranscriptPanel, InterruptButton, InterruptChatPanel, ReferenceList, Teleprompter, ForkAttribution, ForkLineage, ForkRemixModal, ForkGraph, VersionHistory, ListeningQueue, SegmentQuestionBadge, ShareMenu, EmbedCodeModal, EmbedPlayer, CommunityQuestions, QuestionCard, CommentSection, CommentCard, CommentCompose, PostListenRating, ClaimFlagButton, PodcastJsonLd, VideoPlayer, VideoProgress, VideoView, PipelineEditor (Video Storyboard), SegmentNode (StoryboardCard), TransitionConnector | Podcast playback + transcript + fork lineage + version history + Q&A + comments + sharing + embed + rating + claim flagging + structured data + video generation + synced video subtitles + video storyboard (vertical card list with 3-layer progressive disclosure for pre-render review) |
...
| `landing/chapters/` | HeroChapter, JourneyChapter, ShowcaseChapter, TrustChapter, NetworkChapter, ConvertChapter                                                                                                                                                                                                     | Landing page scrollytelling chapters: hero (asymmetric 2-col + CTA), journey (3-step glassmorphism mocks), showcase (video pipeline), trust (verification), network (magazine grid + use cases), convert (pricing + CTA + footer) |
