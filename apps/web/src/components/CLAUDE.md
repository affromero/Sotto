...
| `player/`        | AudioPlayer, MiniPlayer, Waveform, PlaybackControls, TranscriptPanel, InterruptButton, InterruptChatPanel, ReferenceList, Teleprompter, ForkAttribution, ForkLineage, ForkRemixModal, ForkGraph, VersionHistory, ListeningQueue, SegmentQuestionBadge, ShareMenu, EmbedCodeModal, EmbedPlayer, CommunityQuestions, QuestionCard, CommentSection, CommentCard, CommentCompose, PostListenRating, ClaimFlagButton, PodcastJsonLd, VideoPlayer, VideoProgress, VideoView, PipelineEditor (Video Storyboard), SegmentNode (StoryboardCard) | Podcast playback + transcript + fork lineage + version history + Q&A + comments + sharing + embed + rating + claim flagging + structured data + video generation + synced video subtitles + video storyboard (vertical card list with 3-layer progressive disclosure for pre-render review) |
