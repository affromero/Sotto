...
| `player/`        | AudioPlayer, MiniPlayer, Waveform, PlaybackControls, TranscriptPanel, InterruptButton, InterruptChatPanel, ReferenceList, VocabularyList, Teleprompter, ForkAttribution, ForkLineage, ForkRemixModal, ForkGraph, VersionHistory, ListeningQueue, SegmentQuestionBadge, ShareMenu, EmbedCodeModal, EmbedPlayer, CommunityQuestions, QuestionCard, CommentSection, CommentCard, CommentCompose, PostListenRating, PostListenQuiz, ClaimFlagButton, PodcastJsonLd, CostBreakdown, VideoPlayer, VideoProgress, VideoView, VideoModelPicker, PipelineEditor (Video Storyboard), SegmentNode (StoryboardCard), TransitionConnector, VideoEditor, FilmstripBlock, FilmstripStrip, SegmentDetailPanel | Podcast playback + transcript + fork lineage + version history + Q&A + comments + sharing + embed + rating + quiz + claim flagging + structured data + video generation + model picker + synced video subtitles + video storyboard (pre-render: vertical card list; post-render: horizontal filmstrip timeline with detail panel) + vocabulary list (language learning) |
...
| `feed/`          | PodcastCard, FeedGrid, TagFilter, SearchBar, TrendingSection, UserSearchGrid, SuggestedFollows, ActivityFeed, ActivityItem, NewsTab, NewsCard                                                                                                                                                 | Public feed + activity feed + news feed + user discovery + suggested follows                 |
